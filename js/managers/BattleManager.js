...
export default class BattleManager extends EventEmitter {
    ...
    async attack(attacker, defender) {
        ...
        if (advantage === 1) { // Advantage
            damage *= GameConfig.BATTLE.ELEMENTAL_BONUS; // [Refactor] 1.3 -> Config
        } else if (advantage === -1) { // Disadvantage
            damage *= GameConfig.BATTLE.ELEMENTAL_PENALTY; // [Refactor] 0.7 -> Config
        ...
        if (attacker.isHero && this.activeSynergies[WORLDS.OLYMPUS] >= 2) {
            if (Math.random() < 0.20) {
                isCrit = true;
                damage *= GameConfig.BATTLE.CRIT_MULTIPLIER; // [Refactor] 1.5 -> Config
            }
        }
